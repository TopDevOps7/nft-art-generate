...
def parse_config():
    ...
    assets_path ='assets_Medium_Man_SR2'
...
def generate_single_image(filepaths, output_filename=None):
    ...
    for filepath in filepaths[1:]:
        if filepath.endswith('.png'):
            img = Image.open(os.path.join(assets_path, filepath)).convert("RGBA")
...
def generate_trait_set_from_config():
    ...
    if trait_paths is not None:
        hair_back = trait_paths[2]
        Head_Hat = trait_paths[10]
        hat_name = trait_paths[10][9:-4]
        change_name_eye = hat_name.replace("Head_Hat_","eye_Hat_")
        change_name_mouth = hat_name.replace("Head_Hat_","mouth_Hat_")
        replace_eye = change_name_eye[0:-3]
        replace_mouth = change_name_mouth[0:-3]
        num = random.choice (randomlist)
        num1 = random.choice (randomlist)
        replace_eye +="("+str(num)+")"
        replace_mouth +="("+str(num1)+")"
        trait_paths[9] = hair_back.replace("A_", "B_").replace("hairback", "hairfront")
        trait_paths[11] = Head_Hat.replace(hat_name,replace_eye).replace("Head_Hat", "eye_Hat")
        trait_paths[12] = Head_Hat.replace(hat_name, replace_mouth).replace("Head_Hat", "mouth_Hat")
